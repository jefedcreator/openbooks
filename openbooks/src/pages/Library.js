...
const Library = ({ myNfts, spinner, profit }) => {
  ...
  const openbooks = new ethers.Contract(value.address, openbooksAbi, signer);
  ...
  return (
    <div className="flex flex-col justify-between h-3/4 relative w-full">
      ...
      <div className="flex justify-between">
        ...
        >
          {profit
            ? `Claim ${ethers.utils.formatEther(new BigNumber(profit).toString())} FIL`
            : `0 FIL`}
        </button>
      </div>
      <div className="h-full flex flex-row flex-wrap justify-between">
        {myNfts.map((book, i) => {
          return (
            <div
              className="max-w-sm bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 mt-3"
              key={i}
            >
              <img
