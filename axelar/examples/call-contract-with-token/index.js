...
async function test(chains, wallet, options) {
    ...
    for (const chain of [source, destination]) {
        ...
        // console.log("aUSDC", await chain.gateway.tokenAddresses('aUSDC'));
    ...
    async function logAccountBalances() {
        for (const account of accounts) {
            console.log(`${account} has ${(await destination.usdc.balanceOf(account)) / 1e6} aUSDC with receipt: ${await destination.contract.messages(account)}`);
        }
    }
    ...
    console.log("name is:",destination.name)
    console.log("destination is:",destination.distributionExecutable)
